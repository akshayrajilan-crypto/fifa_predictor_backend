...
public class AdminService {
    ...
    /**
     * Resets and recalculates all prediction points for a single match.
     * Called after correcting a score via manual edit or re-pull.
     */
    @Transactional
    public void recalculateMatchPoints(Long matchId) {
        Match match = matchRepository.findById(matchId)
                .orElseThrow(() -> new RuntimeException("Match not found"));

        // Reverse existing scored points from user totals, then reset
        for (Prediction p : predictionRepository.findByMatch(match)) {
            if (p.isScored()) {
                User u = p.getUser();
                u.setTotalPoints(u.getTotalPoints() - p.getPointsEarned());
        ...
        for (GoalScorerPrediction gs : goalScorerPredictionRepository.findByMatch(match)) {
            if (gs.isScored()) {
                User u = gs.getUser();
                u.setTotalPoints(u.getTotalPoints() - gs.getPointsEarned());
                userRepository.save(u);
            }
            gs.setPointsEarned(0);
            gs.setScored(false);
            goalScorerPredictionRepository.save(gs);
        }
        for (MotmPrediction mp : motmPredictionRepository.findByMatch(match)) {
            if (mp.isScored()) {
                User u = mp.getUser();
                u.setTotalPoints(u.getTotalPoints() - mp.getPointsEarned());
                userRepository.save(u);
            }
            mp.setPointsEarned(0);
            mp.setScored(false);
            motmPredictionRepository.save(mp);
        }

        if (match.isOmitted()) return;

        // Re-score match predictions
        calculateScorePoints(match);

        // Re-score goal scorers from match_goal_scorers table
        ...
        if (!actualScorers.isEmpty()) {
            ...
            for (MatchGoalScorer gs : actualScorers) {
                if (!gs.isOwnGoal()) actualGoalsByName.merge(gs.getPlayerName().toLowerCase(), 1, Integer::sum);
            }
            for (GoalScorerPrediction prediction : goalScorerPredictionRepository.findByMatchAndScored(match, false)) {
                int actualGoals = 0;
                for (java.util.Map.Entry<String, Integer> entry : actualGoalsByName.entrySet()) {
                    if (namesMatch(prediction.getPlayer().getName().toLowerCase(), entry.getKey())) {
                        actualGoals = entry.getValue();
                        break;
                    }
                }
                int predictedGoals = prediction.getPredictedGoals();
                int points = actualGoals > 0
                        ? (GOAL_SCORER_POINTS * Math.min(actualGoals, predictedGoals)) + (WRONG_SCORER_PENALTY * Math.max(0, predictedGoals - actualGoals))
                        : WRONG_SCORER_PENALTY * predictedGoals;
        ...
        if (match.getManOfTheMatch() != null && !match.getManOfTheMatch().isBlank()) {
            for (MotmPrediction pred : motmPredictionRepository.findByMatchAndScored(match, false)) {
                if (namesMatch(pred.getPlayer().getName().toLowerCase(), match.getManOfTheMatch().toLowerCase())) {
                ...
                if (pred.getPointsEarned() > 0) {
                    User u = pred.getUser();
                    u.setTotalPoints(u.getTotalPoints() + pred.getPointsEarned());
                    userRepository.save(u);
                }
            }
        }
